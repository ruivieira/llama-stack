...
# Shared build utilities for TrustyAI distributions
...
def get_dependencies(build_config_path: str):
    """Execute the llama stack build command and capture dependencies."""
    cmd = f"llama stack build --config {build_config_path} --print-deps-only"
    try:
...
def generate_containerfile(dependencies: str, distro_dir: str, distro_name: str):
    """Generate Containerfile from template with dependencies."""
    template_path = Path(f"{distro_dir}/Containerfile.in")
    output_path = Path(f"{distro_dir}/Containerfile")
    ...
    warning = (
        f"# WARNING: This file is auto-generated. Do not modify it manually.\n# Generated by: {distro_dir}/build.py\n\n"
    )
...
def build_distribution(distro_dir: str, distro_name: str):
    """Build a specific distribution."""
    print(f"Building {distro_name} distribution...")
    ...
    build_config_path = f"{distro_dir}/build.yaml"
    print(f"Getting dependencies from {build_config_path}...")
    dependencies = get_dependencies(build_config_path)

    print("Generating Containerfile...")
    generate_containerfile(dependencies, distro_dir, distro_name)

    print(f"{distro_name} distribution build complete!")
