        ...
        for line in result.stdout.splitlines():
            if line.strip().startswith("uv pip"):
                ...
                if len(parts) >= 4:  # We have packages to sort
                    ...
                    # Categorize based on command type
                    if "--index-url" in full_cmd:
                        torch_deps.append(full_cmd)
                    elif "--no-deps" in full_cmd:
                        no_deps.append(full_cmd)
                    elif "--no-cache" in full_cmd:
...
def generate_containerfile(dependencies):
    ...
    containerfile_content = warning + template_content.format(dependencies=dependencies)
